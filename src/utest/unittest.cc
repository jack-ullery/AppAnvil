...
TEST(COMMAND_CALLER, LOAD_PROFILE){

	std::string FEEDBACK;

	std::string scmd = "echo \"123\" | sudo -S rm /etc/apparmor.d/usr.bin.certspotter 1>null 2> null";
	system(scmd.c_str());
	scmd = "echo \"123\" | sudo -S aa-remove-unknown 1>null 2>null";
	system(scmd.c_str());	
	//std::string scmd = "aa-easyprof /usr/bin/certspotter > usr.bin.certspotter";
	scmd = "aa-easyprof /usr/bin/certspotter > usr.bin.certspotter";
	system(scmd.c_str());

	CommandCaller::load_profile("/home/fer/Desktop/appNov15/AppAnvil-main/utest/usr.bin.certspotter", "123", FEEDBACK);

	std::vector<std::string> args = {"pkexec", "aa-status"};
  std::string child_output;
  std::string child_error;
  int exit_status = 0;
  Glib::spawn_sync("/usr/sbin/", args, Glib::SpawnFlags::SPAWN_DEFAULT, {}, &child_output, &child_error, &exit_status);

  EXPECT_THAT(child_output, testing::HasSubstr("usr/bin/certspotter"));
