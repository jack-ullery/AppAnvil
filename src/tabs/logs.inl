...
void Logs<ColumnRecord>::add_row_from_line(const std::shared_ptr<ColumnRecord> &col_record, const std::string &line)
{
  ...
  col_record->set_row_data(row, 1, parse_line(line, filter_log_type<ColumnRecord>));
  col_record->set_row_data(row, 2, parse_line(line, filter_log_operation<ColumnRecord>));
  col_record->set_row_data(row, 3, parse_line(line, filter_log_name<ColumnRecord>));
  col_record->set_row_data(row, 4, parse_line(line, filter_log_pid));
  col_record->set_row_data(row, 5, parse_line(line, filter_log_status<ColumnRecord>));
}
